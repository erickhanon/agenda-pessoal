        ...
        <v-card color="surface" style="padding: 0.5em; max-width: 30em;">
          ...
          <v-card-text>
            <v-form @submit.prevent="login">
              <v-text-field variant="outlined" v-model="username" label="Usuário" required></v-text-field>
...
const login = async () => {
  try {
    const response = await fetch('https://demometaway.vps-kinghost.net:8485/api/auth/login', {
      method: 'POST',
      headers: {
