<template>
  <v-app-bar app color="info" dark density="compact" >
    <v-app-bar-nav-icon @click="drawer = !drawer" class="d-md-none" :style="{ color: 'white' }"></v-app-bar-nav-icon>
    ...
    <div class="d-none d-md-flex">
      <v-tooltip bottom v-for="item in filteredMenuItems" :key="item.title">
        <template v-slot:activator="{ props }">
          <NuxtLink v-if="item.to" :to="item.to" class="v-btn v-btn--icon" v-bind="props" :style="{ color: 'white', marginLeft: '1.2rem'  }">
            <v-icon>{{ item.icon }}</v-icon>
          </NuxtLink>
          <v-btn v-else icon @click="handleAction(item)" v-bind="props" :style="{ color: 'white' }">
            <v-icon>{{ item.icon }}</v-icon>
          </v-btn>
        </template>
  ...
  <v-navigation-drawer v-model="drawer" app temporary class="d-md-none" :style="{ width: '72px' }">
    <v-list dense>
      <v-tooltip bottom v-for="item in filteredMenuItems" :key="item.title">
        <template v-slot:activator="{ props }">
          <v-list-item @click="handleAction(item)" v-bind="props" :style="{ justifyContent: 'center' }">
            <v-icon>{{ item.icon }}</v-icon>
          </v-list-item>
        </template>
        <span>{{ item.title }}</span>
